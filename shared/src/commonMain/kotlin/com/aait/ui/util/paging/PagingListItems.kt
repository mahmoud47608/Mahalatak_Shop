...
fun PagingErrorItem(
    ...
    Column(
        ...
        Text(
            text = message ?: stringResource(Res.string.something_went_wrong),
...
fun <T : Any> LazyListScope.pagingEndOfListItem(
    ...
    if (appendState is LoadState.NotLoading && appendState.endOfPaginationReached && pagingItems.itemCount > 0) {
        item(key = "paging_end_of_list") {
            PagingEndOfListIndicator(message = message ?: stringResource(Res.string.end_of_list))
        }
    }
}
...
fun PagingEndOfListIndicator(
    modifier: Modifier = Modifier,
    message: String = stringResource(Res.string.end_of_list)
) {
    Box(
        modifier = modifier
            .fillMaxWidth()
