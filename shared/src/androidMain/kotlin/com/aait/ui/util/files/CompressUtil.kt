package com.aait.ui.util.files
    ...
    suspend fun compressVideoToCache(context: Context, inputFilePath: String?): String? {
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                Log.d(
                    "Compress",
                    "Video Compression - Original file size: %.2f MB".format(originalSize)
                )
                ...
                val cacheDir = context.cacheDir
                val outputFile =
                    File(cacheDir, "compressed_video_${System.currentTimeMillis()}.mp4")
                ...
                Log.d(
                    "Compress",
                    "Video Compression - Compressed file size: %.2f MB".format(compressedSize)
                )
