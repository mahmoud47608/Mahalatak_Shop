            ...
            scope.launch {
                ...
                try {
                    ...
                    uris.forEach { uri ->
                        ...
                        if (!compressed.isNullOrEmpty()) compressedPaths.add(compressed)
                    }
                    if (compressedPaths.isNotEmpty()) onMediaSelected(compressedPaths)
                } finally {
                    isLoading = false
                }
            }
        ...
        if (success && filePath != null && fileToCompress?.exists() == true) {
            scope.launch {
                ...
                try {
                    ...
                    if (!compressed.isNullOrEmpty()) onMediaSelected(listOf(compressed))
                } catch (e: Exception) {
                    e.printStackTrace()
                } finally {
                    isLoading = false
